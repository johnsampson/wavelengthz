...
- **No self-inclusion guardrail.** An earlier version of this feature rejected any saved `age_min`/`age_max` that excluded the caller's own age, on the theory that such a range is a hallmark of predatory targeting. Removed by explicit product decision -- a saved age range is a search preference, not a claim about the user's own age, and is enforced as a genuine candidate-search filter (`GET /api/candidates/people`), not merely a scoring input.
