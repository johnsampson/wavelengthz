...
### UX gaps closed post-launch (display names, match celebration, notifications, previews)

Five follow-on features, identified by reviewing the live app for missing UX rather than from a bug report. All TDD'd, full suite green, `tsc` clean throughout.

- **Required display names**: `users.display_name` (already existed in schema) is now a required onboarding field — `src/routes/onboarding.ts` validates and trims it, returning `invalid_display_name` on empty/whitespace input. Collected in `public/onboarding.html` and editable afterward via a new field in `public/settings.html`/`public/settings.js`. The deck and profile pages already rendered `displayName`; this closes the gap where it could be blank.
- **Match celebration in the deck**: `POST /api/swipe/people` and `PATCH /api/swipes/people/:id` now return `matched`/`matchId` in their response (previously silent). `public/index.html`'s `deckApp()` shows a full-screen "It's a match!" modal with the other person's photo/name and a "Send a Message"/"Keep Swiping" choice instead of instantly advancing to the next candidate — matches were previously only discoverable later, on the Matches tab.
- **Notifications UI**: `GET /api/notifications` (`src/routes/notifications.ts`) now resolves and returns a `matchId` for both `match`- and `message`-type rows (via a `LEFT JOIN`, camelCased response fields) so the UI can deep-link. New page `public/notifications.html` lists all notifications with read/unread state and navigates to `/match?id=` on click, marking read via the existing `POST /api/notifications/:id/read`. The shared header (`public/nav.js`) gained a bell icon with an unread-count badge (capped display at "9+"), fetched via a new `api.notifications()` call in `public/app.js`.
- **Track preview playback**: `public/artist.html` adds a play/pause button per track (uses the `previewUrl` already returned by `GET /api/artists/:id`, previously unused) backed by a single shared `<audio>` element — clicking a second track's play button stops the first. No backend change.
- **Self-profile preview**: `GET /api/people/:id/profile` (`src/routes/peopleSwipes.ts`) previously 400'd (`cannot_view_self`) when `:id` was the caller's own id. It now allows self-view, returning `isSelf: true` and skipping the blocked-check and music-overlap computation (both meaningless against yourself). `public/profile.html` hides the overlap sections and likedYou/isMatch badges when `isSelf` is true, showing a "Your Profile" heading instead. A "Preview my profile" link was added to `public/settings.html`, using the caller's own id (`public/settings.js` now loads `userId` from `/api/me` on init).

### Age range search preference

- **Settings-only, dual-thumb slider** (`public/settings.html`/`public/settings.js`): `age_min`/`age_max` (18-100, migration `0004_add_age_range.sql`, default full range) built from two stacked native `<input type="range">`s so dragging keeps real keyboard/touch support instead of hand-rolled pointer logic. Styled in `public/styles.css` to pixel-match the existing max-distance slider's native `accent-brand-500` look (same `#3b3b3b` track, same `rgba(255,255,255,0.5)` border, same vertical centering on the thumb row) — deliberately not a `bg-surface2` custom color or brand/accent two-tone gradient, so the two sliders read as one component family.
- **Deliberately left off onboarding.** The slider only exists in Settings, reachable after a profile already exists — not part of the initial signup flow. Keeps first-run onboarding focused on identity/safety basics (DOB, gender, seeking, location) rather than a preference that's easy to revisit later.
- **Safety guardrail — a search range must include the searcher's own age.** `POST /api/onboarding` (shared by both onboarding and the Settings save) now rejects any `age_min`/`age_max` where the caller's own `computeAge(date_of_birth)` falls outside `[age_min, age_max]`, returning `age_range_excludes_self` (400). Rationale: a range that excludes the searcher's own age (e.g. a 45-year-old searching only 18-25) is a hallmark of predatory targeting rather than a genuine preference, so it's rejected outright rather than merely discouraged. Since onboarding never sends `age_min`/`age_max`, this check is a no-op there and only bites on a Settings save. Tests in `test/routes/onboarding.test.ts` (`describe('age range')`) cover both the rejection and the accepted edge case (range starting exactly at the user's own age).

### Future ideas (not scheduled)

- **Paid background checks**: optional, opt-in third-party background-check integration (e.g. a Checkr-style API) surfaced as a verified-safety badge on a user's profile. Explicitly "very future" and framed as trust-and-safety tooling, not a monetization lever — the product stays free and non-monetized by explicit decision; adoption and safety come first.
