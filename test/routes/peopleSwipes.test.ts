...
describe('age_min/age_max is a stored preference only, not a candidate-search filter', () => {
  // Regression: an earlier version enforced this range the same way
  // max_distance_km is enforced, which excluded candidates outside it for
  // every intent -- including "making friends," where restricting by age
  // isn't wanted. age_min/age_max is saved (see onboarding.test.ts) but must
  // never narrow who shows up here.
  async function makeUserWithAge(id: string, dateOfBirth: string, ageMin: number, ageMax: number) {
    await env.DB.prepare(
  ...
  it('includes a candidate outside the caller\'s age range', async () => {
    await env.DB.prepare('UPDATE users SET age_min = 30, age_max = 40 WHERE id = ?').bind('u1').run();
    await makeUserWithAge('young', '2005-06-01', 18, 100); // 20 years old, well outside 30-40

    const cookie = await cookieFor('u1');
    const res = await worker.fetch(new Request('http://localhost/api/candidates/people?limit=50', { headers: { Cookie: cookie } }), env, {} as ExecutionContext);
    const body = await res.json<any>();
    expect(body.candidates.find((c: any) => c.id === 'young')).toBeDefined();
  });

  it('includes an out-of-range liker in the like-priority queue too', async () => {
    await env.DB.prepare('UPDATE users SET age_min = 30, age_max = 40 WHERE id = ?').bind('u1').run();
    await makeUserWithAge('young-liker', '2005-06-01', 18, 100); // 20 years old
    await env.DB.prepare(
    ...
    const res = await worker.fetch(new Request('http://localhost/api/candidates/people', { headers: { Cookie: cookie } }), env, {} as ExecutionContext);
    const body = await res.json<any>();
    expect(body.candidates.find((c: any) => c.id === 'young-liker')).toBeDefined();
  });
});
