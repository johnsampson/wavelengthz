...
describe('POST /api/onboarding', () => {
  ...
  describe('age range', () => {
    ...
    it('accepts a range that excludes the user\'s own age', async () => {
      // No self-inclusion guardrail: a saved age range is a search
      // preference, not a claim about the user's own age.
      const cookie = await sessionCookieFor('u1');
      const res = await post(cookie, completePayload({ date_of_birth: '1995-01-01', age_min: 18, age_max: 25 }));
      expect(res.status).toBe(200);
      const row = await env.DB.prepare('SELECT age_min, age_max FROM users WHERE id = ?').bind('u1').first<any>();
      expect(row.age_min).toBe(18);
      expect(row.age_max).toBe(25);
    });
  });
});
